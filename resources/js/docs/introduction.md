...
Peermitly is a blazing-fast local development environment for **macOS, Windows and Linux**. Drop a project into your folder and it is instantly served on a secure `.test` domain — no config files, no containers, no terminal gymnastics.
...
- **Instant sites** — a project becomes a live `.test` URL in under a second.
...
```bash
# Your project folder
~/code/my-app        ->  https://my-app.test
~/code/api-service   ->  https://api-service.test
```
